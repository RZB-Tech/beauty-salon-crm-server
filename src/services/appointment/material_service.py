...
from src.exceptions.general_exceptions import CannotUpdate, FieldCannotBeEmpty, ValueCannotBeNegative
...
class MaterialService():
    ...
    async def update(self, data: MaterialUpdateSchema) -> Material:
        checkIfExists = await self.uow.materials.get(data.id)
        if checkIfExists is None: raise MaterialNotFound(data.id)

        dataDict = data.model_dump(exclude={"id"}, exclude_unset=True)
        
        for key in ["article", "name"]:
            if key in dataDict and not dataDict[key]: raise FieldCannotBeEmpty(key)

        result = await self.uow.materials.update(data.id, **dataDict)
        if result is None: raise CannotUpdate(data.id, "materials")
        return result
    
    async def get(self, id: int) -> Material:
        result = await self.uow.materials.get(id)
        if result is None: raise MaterialNotFound(id)
        return result
    ...
    async def updateQuantity(self, data: MaterialQuantityUpdateSchema) -> Material:
        ...
        if material is None: raise MaterialNotFound(data.id)
        
        newQuantity = material.quantity + (data.operation * data.quantity)
        if newQuantity < 0: raise ValueCannotBeNegative()
